...
% m=100;
% grid = linspace(0,1,m);
% [x,y] = meshgrid(grid); dsites = [x(:) y(:)];
  pts=haltonseq(20000,2);
  dsites=pts(1:10000,:); 
  epoints=pts(10001:10000+1500,:);
  ctrs = dsites; 
  %neval = 40; grid = linspace(0,1,neval);
  %[xe,ye] = meshgrid(grid); epoints = [xe(:) ye(:)];
%  nt=1550; epoints=haltonseq(nt,2);
  ...
  DM_data = DistanceMatrixCSRBF_B(dsites,ctrs,ep);
  % Compute interpolation matrix
  IM = rbf(ep,DM_data);
  % Compute distance matrix between evaluation points and centers
  DM_eval = DistanceMatrixCSRBF_B(epoints,ctrs,ep);
