function [index_vals,dist_vals,vector_vals] = kd_rangequery_ball(tree,point,range,node_number)

% pramod vemulapalli 02/07/2010
% Modified to work with 2-norm range queries by Greg Fasshauer 09/18/2010
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% INPUTS
% tree        --- the cell array that contains the tree 
% point       --- the point of interest 
% range       --- distance from the point 
...
% ORIGINAL CODE - allows only use of max-norm distance for range
% calculation
%if (sum(tree_cell(node_number).nodevector>=point+range(1,:))==dim && ...
%        sum(tree_cell(node_number).nodevector<=point+range(2,:))==dim)
% NEW CODE - can use any norm for range calculation
distance = norm(tree_cell(node_number).nodevector-point,2);
if distance<=range
...
if ( ((point(tree_cell(node_number).splitdim)-range)<=tree_cell(node_number).splitval) && ...
        ((point(tree_cell(node_number).splitdim)+range)<=tree_cell(node_number).splitval) )

    if (~isempty(tree_cell(node_number).left))
        [index_vals1,dist_vals1,vector_vals1]=kd_rangequery_ball(0,point,range,tree_cell(node_number).left);
    else
...
%    if (~isempty(tree_cell(node_number).left))
    if (~isempty(tree_cell(node_number).right))
        [index_vals1,dist_vals1,vector_vals1]=kd_rangequery_ball(0,point,range,tree_cell(node_number).right);
    else
        index_vals1=[];dist_vals1=[];vector_vals1=[];
    end
    index_vals=[index_vals;index_vals1];
...
if ( ((point(tree_cell(node_number).splitdim)-range)<=tree_cell(node_number).splitval) &&...
    ...
    if (~isempty(tree_cell(node_number).right))
        [index_vals2,dist_vals2,vector_vals2]=kd_rangequery_ball(0,point,range,tree_cell(node_number).right);
    else
