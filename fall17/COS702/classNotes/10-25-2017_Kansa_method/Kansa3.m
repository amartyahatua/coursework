...
warning off all;
pde =@ (r,c)  c^2*(2+c^2*r.^2)./(1+r.^2*c^2).^(3/2); %(r.^2+2*c^2)./(r.^2+c^2).^(3/2);  
rbf=@(r,c) sqrt(1+r.^2*c^2); %sqrt(r.^2+c.^2);
n=200;  M=150;   % n: # of boundary points;  M:# of interior points
t=linspace(0,2*pi,n); 
 r=curve(t');
 t=t+.5*sin(4*t);  %gear shape
x=r.*cos(t'); y=r.*sin(t');  %boundary points
%%%%%%%%%%%%%%%%%%%%
b = max(x); a = min(x); c1 = min(y); d = max(y);  %bounding box 
p=haltonset(2);
pts=net(p,2000);  % 2000 Halton points
pts(:,1)=pts(:,1)*(b-a)+a; pts(:,2)=pts(:,2)*(d-c1)+c1; 
...
rhs=[f;g]; 
%LOOCV
 [c,~] = fminbnd(@(ep) CostEps(ep,pde,rbf,DM1,DM2,rhs),0,29);
A1=pde(DM1,c);   A2=rbf(DM2,c); 
coe=[A1;A2]\rhs;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
b = max(x)+.1; a = min(x)-.1; c1 = min(y)-.1; d = max(y )+.1;
L=80;
h = a:(b-a)/L:b; k = c1:(d-c1)/L:d;
[X, Y] = meshgrid(h,k);   
DM2 = DistanceMatrix([X(:) Y(:)], coll);
Pf=rbf(DM2,c)*coe;
...
%   Plot data sites with interpolant (zero contour of 2D-fit Pf)
  figure (2)
  contour(X,Y,reshape(Pf,L+1,L+1),[1 1],'r','LineWidth',3);
  hold on;
  plot(interior(:,1),interior(:,2),'bo');
  hold off;  axis equal;