...
%%Exact area of Amoeba%%
  ...
  areaAmoeba = zeros(31,200);
  minError = 100;
  errorSet = zeros(31,200);
  ...
  for totalPoints = 100:100:20000
      ...
      Pf_hlt = EM_hlt * coe;
      area = (bmax(1)-bmin(1)).*(bmax(2)-bmin(2));
      pointsInSide = sum(sum(Pf_hlt<=0));
      
      areaAmoeba(((N-50)/5+1),totalPoints/100) = area*(pointsInSide/totalPoints);
      errorSet(((N-50)/5+1),totalPoints/100) = abs(exactArea-(area*(pointsInSide/totalPoints)));
     
      if (areaError > abs(exactArea-(area*(pointsInSide/totalPoints))))
          leastAreaError = abs(exactArea-(area*(pointsInSide/totalPoints)));
          approxArea = area*(pointsInSide/totalPoints);
  ...
  minep = 60;
  maxep = 100;
  %rhs = [zeros(200,1); ones(200,1); -ones(200,1)];
  [ep1,fval] = fminbnd(@(ep) CostEps(ep,rbf,DM_data,rhs),minep,maxep);
  ...
  fprintf('Value of C calculated by LOOCV is %f',ep1);
